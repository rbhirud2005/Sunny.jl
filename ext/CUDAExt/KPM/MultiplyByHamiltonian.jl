...
function multiply_by_hamiltonian_dipole_device_kernel!(Y, X, swt, qs_reshaped, regularization, L)
...
function multiply_by_hamiltonian_dipole_device!(
    y::CUDA.CuMatrix{ComplexF64},
    x::CUDA.CuMatrix{ComplexF64},
    swt::SpinWaveTheoryDevice,
    qs_reshaped::CUDA.CuVector{Sunny.Vec3},
)
    ...
    kernel = CUDA.@cuda launch=false multiply_by_hamiltonian_dipole_device_kernel!(
        Y3, X3, swt, qs_reshaped, swt.regularization, L
    )
    config = launch_configuration(kernel.fun)
    threads = Base.min(Nq, config.threads)
    blocks = cld(Nq, threads)
    kernel(Y3, X3, swt, qs_reshaped, swt.regularization, L; threads, blocks)
...
function Sunny.mul_dynamical_matrix!(
    ...
    multiply_by_hamiltonian_dipole_device!(y, x, swt, qs_reshaped)
    return y
end
