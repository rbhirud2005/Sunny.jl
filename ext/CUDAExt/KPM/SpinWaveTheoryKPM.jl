...
"""
    SpinWaveTheoryKPMDevice

Device-side wrapper around `Sunny.SpinWaveTheoryKPM`. Carries:
- `swt_d :: SpinWaveTheoryDevice` — device LSWT
- `swt_host :: Sunny.SpinWaveTheory` — host LSWT, kept for `Sunny.eigbounds`
  calls (CPU-only) and for accessing `swt_host.measure` from the
  host-side projection block
- `tol :: Float64`, `niters :: Int`, `niters_bounds :: Int`, `method :: Symbol`
  — copied verbatim from CPU `SpinWaveTheoryKPM`

All fields are concrete-typed for type stability of closures
that capture this struct.
"""
struct SpinWaveTheoryKPMDevice
...
function Adapt.adapt_structure(to, swt_kry_d::SpinWaveTheoryKPMDevice)
...
function Sunny.to_device(swt_kry::Sunny.SpinWaveTheoryKPM)
    ...
    return SpinWaveTheoryKPMDevice(
        swt_d,
        swt_kry.swt,
        swt_kry.tol,
        swt_kry.niters,
...
function Sunny.intensities(swt_kry_d::SpinWaveTheoryKPMDevice, qpts;
...
function Sunny.intensities!(data, swt_kry_d::SpinWaveTheoryKPMDevice, qpts;
                            energies, kernel::Sunny.AbstractBroadening,
                            kT=0.0, verbose=false)
    qpts = convert(Sunny.AbstractQPoints, qpts)
    @assert size(data) == (length(energies), size(qpts.qs)...)
    return intensities_lanczos_device!(data, swt_kry_d, qpts;
                                        energies, kernel, kT, verbose)
end

function intensities_lanczos_device!(data, swt_kry_d::SpinWaveTheoryKPMDevice, qpts;
                                     energies, kernel, kT, verbose)
    ...
    q_reshaped_all_d = CUDA.CuVector(q_reshaped_host)   

    v   = CUDA.zeros(ComplexF64, twoL)
    vp  = CUDA.zeros(ComplexF64, twoL)
    Sv  = CUDA.zeros(ComplexF64, twoL)
    Svp = CUDA.zeros(ComplexF64, twoL)
    w   = CUDA.zeros(ComplexF64, twoL)
    Sw  = CUDA.zeros(ComplexF64, twoL)

    lhs_dev = CUDA.zeros(ComplexF64, twoL, Nobs)

    corr_dev  = CUDA.zeros(ComplexF64, Nobs)
    corr_host = Vector{ComplexF64}(undef, Nobs)

    max_iters_global = max(twoL - 1, 1)
